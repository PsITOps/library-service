...
describe('book.js', () => {
                    ...
                    .send({
                        ...
                        User.findById(user._id)
                            .then(foundUser => {
                                ...
                                expect(foundUser.books[0]).to.have.property('returnDate')
                                expect(foundUser.books[0].returnDate).to.be.not.undefined;
                ...
                chai.request(service)
                    ...
                    .end((err, res) => {
                        ...
                        Book.findOne({
                            ...
                            .then(foundBook => {
                                expect(foundBook._doc).to.have.property('isAvailable', true);
                                done();
                            })
                    })
            });
                        ...
                        chai.request(service)
                            ...
                            .end((err, res) => {
                                ...
                                User.findOne({
                                    ...
                                    expect(foundUser.books).to.be.an('Array')
                                    expect(foundUser.books).to.have.lengthOf(0)


                                    return Book.findById(book._id)
                                }).then(foundBook => {
                                    expect(foundBook).to.have.property('isAvailable', true);
                                    done();
                                })
    ...
    describe('POST /books/:id/rent/extend', () => {
        beforeEach((done) => {
            Book.remove({})
                .then(() => {
                    return User.remove({})
        ...
        it('should failed to extend the rent without token provided', (done) => {
            chai.request(service)
                .post('/api/books/1/rent/extend')
                .end((err, res) => {
                    expect(res).to.has.status(401);
                    expect(res.body).to.has.property('valid', false);
                    expect(res.body).to.has.property('message');
        ...
        describe('Authorized requests', () => {
            ...
            it('should failed to extend the rent with unkown identificator of book', (done) => {

                Book.remove({})
                    .then(() => {
                        chai.request(service)
                            .post(`/api/books/${book._id}/rent/extend`)
                            .send({
                                token: token
                            })
                            .end((err, res) => {
            ...
            it('should failed to extend the rent of book that has not been rent by the logged user', (done) => {
                chai.request(service)
                    .post(`/api/books/${book._id}/rent/extend`)
                    .send({
                        token: token
            ...
            it('should successful extend the rent of a book', (done) => {
                chai.request(service)
                    .post(`/api/books/${book._id}/rent`)
                    .send({
                        token: token
                    ...
                    .end((err, res) => {
                        chai.request(service)
                            ...
                            .end((err, res) => {
                                expect(res).to.has.status(200);
                                expect(res.body).to.has.property('valid', true);
                                expect(res.body).to.has.property('returnDate');
